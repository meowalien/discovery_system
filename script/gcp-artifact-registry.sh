...
kubectl create secret docker-registry gcp-artifact-registry --docker-server=https://asia-east1-docker.pkg.dev --docker-email="argocd-service-accountt@$PROJECT_ID.iam.gserviceaccount.com" --docker-username=argocd --docker-password="$(cat ~/argocd-service-account-key.json)"
